...
use hello_world_utils::{
    ...
    depegservicemanager::{DepegServiceManager, IDepegServiceManager::Task},
...
async fn sign_and_response_to_task(
    ...
    let hello_world_contract_address: Address =
        parse_hello_world_service_manager("contracts/deployments/depeg/31337.json")?;
    let hello_world_contract = DepegServiceManager::new(hello_world_contract_address, &pr);
...
async fn monitor_new_tasks() -> Result<()> {
    ...
    loop {
        ...
        for log in logs {
            match log.topic0() {
                Some(&DepegServiceManager::NewTaskCreated::SIGNATURE_HASH) => {
                    let DepegServiceManager::NewTaskCreated { taskIndex, task } = log
                        .log_decode()
...
async fn register_operator() -> Result<()> {
    ...
    let stake_registry_address: Address =
        parse_stake_registry_address("contracts/deployments/depeg/31337.json")?;
...
#[allow(unused)]
/// Calls CreateNewTask function of the Depeg service manager contract
async fn create_new_task(task_name: &str) -> Result<()> {
    ...
    let s = &format!(
        "{}/contracts/deployments/depeg/31337.json",
        &path.display()
    );
    let parsed: DepegData =
...
mod tests {
    ...
    use std::path::Path;
    use DepegServiceManager::latestTaskNumReturn;
    #[tokio::test]
    #[serial]
    ...
    async fn test_spam_tasks() {
        ...
        let parsed: DepegData = serde_json::from_str(&data).unwrap();
        ...
        let hello_world_contract =
            DepegServiceManager::new(hello_world_contract_address, provider);
