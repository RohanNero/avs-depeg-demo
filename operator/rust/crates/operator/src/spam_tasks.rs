...
use eyre::Result;
use hello_world_utils::{depegservicemanager::DepegServiceManager, DepegData};
...
async fn create_new_task(task_name: &str) -> Result<()> {
    let data = std::fs::read_to_string("contracts/deployments/depeg/31337.json")?;
    let parsed: DepegData = serde_json::from_str(&data)?;
