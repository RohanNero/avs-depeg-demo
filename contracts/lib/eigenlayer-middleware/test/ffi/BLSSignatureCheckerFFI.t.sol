...
contract BLSSignatureCheckerFFITests is MockAVSDeployer, G2Operations {
    ...
    bytes32 msgHash = keccak256(abi.encodePacked("depeg"));
    ...
    function testBLSSignatureChecker_SingleQuorum_Valid(
        ...
        uint256 numNonSigners = pseudoRandomNumber %
            (maxOperatorsToRegister - 1);
        ...
        (
            ...
            BLSSignatureChecker.NonSignerStakesAndSignature
    ...
    function testBLSSignatureChecker_100Quorums_Valid(
        ...
        nonSignerStakesAndSignature.sigma = sigma.scalar_mul(
    ...
    function _generateSignerAndNonSignerPrivateKeys(
        ...
        signerPrivateKeys[numSigners - 1] = addmod(
            aggSignerPrivKey,
            BN254.FR_MODULUS - (sum % BN254.FR_MODULUS),
            BN254.FR_MODULUS
        );
    ...
    function _registerSignatoriesAndGetNonSignerStakeAndSignatureRandom(
        ...
        internal
        returns (uint32, BLSSignatureChecker.NonSignerStakesAndSignature memory)
    {
        (
            uint256[] memory signerPrivateKeys,
            uint256[] memory nonSignerPrivateKeys
        ) = _generateSignerAndNonSignerPrivateKeys(
                pseudoRandomNumber,
                maxOperatorsToRegister - numNonSigners,
                numNonSigners
            );
        bytes memory quorumNumbers = BitmapUtils.bitmapToBytesArray(
            quorumBitmap
        ...
        BN254.G1Point[] memory pubkeys = new BN254.G1Point[](
            maxOperatorsToRegister
        );
        BLSSignatureChecker.NonSignerStakesAndSignature
            memory nonSignerStakesAndSignature;
        nonSignerStakesAndSignature.quorumApks = new BN254.G1Point[](
            quorumNumbers.length
        );
        nonSignerStakesAndSignature.nonSignerPubkeys = new BN254.G1Point[](
            numNonSigners
        );
        bytes32[] memory nonSignerOperatorIds = new bytes32[](numNonSigners);
        {
            ...
            for (uint i = 0; i < maxOperatorsToRegister; i++) {
                uint256 randomSeed = uint256(
                    keccak256(abi.encodePacked("privKeyCombination", i))
                );
                if (
                    randomSeed % 2 == 0 &&
                    signerIndex < signerPrivateKeys.length
                ) {
                    ...
                    nonSignerOperatorIds[
                        ...
                        .nonSignerPubkeys[nonSignerIndex]
                        .hashG1Point();
                    nonSignerIndex++;
                } else {
                    privateKeys[i] = signerPrivateKeys[signerIndex];
                ...
                for (
                    uint j = 0;
                    j < nonSignerStakesAndSignature.quorumApks.length;
                    j++
                ) {
                    nonSignerStakesAndSignature.quorumApks[
                            j
                        ] = nonSignerStakesAndSignature.quorumApks[j].plus(
                        pubkeys[i]
                    );
                }
            }
        ...
        uint32 referenceBlockNumber = registrationBlockNumber +
            blocksBetweenRegistrations *
            uint32(maxOperatorsToRegister) +
            1;
        cheats.roll(referenceBlockNumber + 100);

        OperatorStateRetriever.CheckSignaturesIndices
            memory checkSignaturesIndices = operatorStateRetriever
        ...
        nonSignerStakesAndSignature
            .nonSignerQuorumBitmapIndices = checkSignaturesIndices
            .nonSignerQuorumBitmapIndices;
        nonSignerStakesAndSignature.apkG2 = aggSignerApkG2;
        nonSignerStakesAndSignature.sigma = sigma;
        nonSignerStakesAndSignature.quorumApkIndices = checkSignaturesIndices
            .quorumApkIndices;
        nonSignerStakesAndSignature.totalStakeIndices = checkSignaturesIndices
            .totalStakeIndices;
        nonSignerStakesAndSignature
