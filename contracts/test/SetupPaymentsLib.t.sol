...
import "../script/utils/DepegDeploymentLib.sol";
...
import "../script/DepegDeployer.s.sol";
import {StrategyFactory} from "@eigenlayer/contracts/strategies/StrategyFactory.sol";
import {DepegTaskManagerSetup} from "test/DepegServiceManager.t.sol";
...
contract SetupPaymentsLibTest is Test, TestConstants, DepegTaskManagerSetup {
    ...
    function testSubmitRoot() public {
        ...
        uint32 endTimestamp = rewardsCoordinator
            .currRewardsCalculationEndTimestamp() + 1 weeks;
    ...
    function testParseLeavesFromJson() public {
        ...
        string
            memory jsonContent = '{"leaves":["0x1234"], "tokenLeaves":["0x5678"]}';
        vm.writeFile(filePath, jsonContent);

        SetupPaymentsLib.PaymentLeaves memory paymentLeaves = SetupPaymentsLib
            .parseLeavesFromJson(filePath);

        assertEq(paymentLeaves.leaves.length, 1, "Incorrect number of leaves");
        assertEq(
            paymentLeaves.tokenLeaves.length,
            1,
            "Incorrect number of token leaves"
        );
    }

    function testGenerateMerkleProof() public {
        SetupPaymentsLib.PaymentLeaves memory paymentLeaves = SetupPaymentsLib
            .parseLeavesFromJson("test/mockData/scratch/payments.json");
        ...
        bytes memory proofBytesCalculated = SetupPaymentsLib
            .generateMerkleProof(leaves, indexToProve);
    ...
    function testProcessClaim() public {
        ...
        cheats.startPrank(
            earnerLeaves[INDEX_TO_PROVE].earner,
            earnerLeaves[INDEX_TO_PROVE].earner
        );
